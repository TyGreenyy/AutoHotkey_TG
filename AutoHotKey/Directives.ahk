...
SetWorkingDir %A_ScriptDir%\AutoHotkey 	;Set script path as working directory. Ensures a consistent starting directory.

; ;~ reloadAsAdmin_Task()			;Runs reloadAsAdmin task

global SCR_Name					;Sets path for tray menu options | SCR_Name = Name of file without extension
...
DetectHiddenWindows, On			;Detect Windows that are hidden? Recommended to set whenever needed
...
#KeyHistory 250 				;maximum number of keyboard and mouse events displayed by the KeyHistory window.
...
#MaxThreads 255					;Allows # of pseudo-threads to run simultaneously. Disable if the script uses too much resources
...
DoubleClickTime := (DllCall("GetDoubleClickTime")/1000)
SetNumLockState, On

;~ x: 958	y: 0	w: 964	h: 1042
